...
function begin_play(){
	if (game_started())
	{
	alert("Already Started, press reset play to start again");
	return;
	}
	...
	if(isEmpty(player1))
	{
		alert("Player 1 must enter a name to play");
	}
	if(isEmpty(player2))
	{
		alert("Player 2 must enter a name to play");
	}
	started = true;
	document.getElementById("player1_id").value.disabled = true;
	document.getElementById("player2_id").value.disabled = true;
	document.getElementById("player1_id").value = `${player1} ${"(X)"}`; //assing x to p1
	document.getElementById("player2_id").value = `${player2} ${"(O)"}`; // assign o to p2
	if (turn == 1)
	{
...
function reset_play(){
	...
	document.getElementById("turn_info").innerText = "The game has not started";
	for(var count=0; count < 9; count++)
	{
		document.getElementById(table_ids[count]).innerText = table_ids[count];
		board_state[count] = -1;
	}
}
...
function play() {
	var count=0;
	let loc = document.getElementById("move_text_id").value;
	if((loc[0] == (/[A-C]/g)) && (loc[1] == (/[1-3]/g)));
	{
		let index = table_ids.indexOf(loc);
		if(board_state[index] != -1)
		{
			alert("That space is already taken please choose again!");
		}
		else if(turn == 1)
		{
			board_state[index] = 1;
			document.getElementById(loc).innerText = "X";
			document.getElementById("move_text_id").value = "";
			document.getElementById("turn_info").innerText = "Player 2's time to act";
			count++;
		}
		else if(turn == 0)
		{
			board_state[index] = 0
			document.getElementById(loc).innerText = "O";
			document.getElementById("move_text_id").value = "";
			document.getElementById("turn_info").innerText = "Player 1's time to act";
			count++;
		}
		if((board_state[0] != -1)&&(board_state[4]!=-1)&&(board_state[8]!=-1)&&(board_state[0] == board_state[4]) && (board_state[0] == board_state[8])||(board_state[0] != -1)&&(board_state[3]!=-1)&&(board_state[6]!=-1)&&(board_state[0] == board_state[3]) && (board_state[0] == board_state[6])||(board_state[0] != -1)&&(board_state[1]!=-1)&&(board_state[2]!=-1)&&(board_state[0] == board_state[1]) && (board_state[0] == board_state[2]))
		{
			if(board_state[0] == 1)
			{
				alert("Player 1 wins.... this time");
				reset_play();
			}
			else
			{
				alert ("Winner Winner chicken dinner for Player 2");
				reset_play();
			}
		}
		...
		else if((board_state[2] != -1)&&(board_state[4]!=-1)&&(board_state[6]!=-1)&&(board_state[2] == board_state[4]) && (board_state[2] == board_state[6])||(board_state[2] != -1)&&(board_state[5]!=-1)&&(board_state[8]!=-1)&&(board_state[2] == board_state[5]) && (board_state[2] == board_state[8]))
		{
			if(board_state[2] == 1)
			{
				alert("Player 1 wins.... this time");
		...
		{
			alert("The game has ended in a tie, time to run it back");
			reset_play();
		}
		else
		{
